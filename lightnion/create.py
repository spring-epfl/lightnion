...
async def fast_async(link):
    ...
    try:
        or_cell = lnn.cell.created_fast.cell(await link.get_async(dummy))
    except KeyError:
    ...
    await link.register_async(final)
...
def ntor_handshake(link, circuit_id, handshake, timeout=None):
    link.send(handshake)
    ...
    return cell.created2.data


async def ntor_async(link, descriptor):
    ...
    circuit_id, payload = await ntor_raw_async(link, payload)
...
def ntor(link, descriptor):
    ...
    circuit_id, handshake = ntor_raw(link, payload)
    payload = ntor_handshake(link, circuit_id, handshake)
